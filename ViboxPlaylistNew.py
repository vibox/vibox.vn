	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.Vibox.vn/(.+?)/.+?\://", item["path"]):
				match = re.search(
					"plugin.video.Vibox.vn(/.+?/).+?\://", item["path"])
	...
	try:
		...
		try:
			...
			yes = dialog.yesno(
				...
				'[COLOR yellow]Hãy nhập tài khoản VIP Fshare của bạn vào, nếu chưa có hãy liên hệ [COLOR blue]facebook.com/hieuhien.vn[/COLOR] để mua giá chiết khấu[/COLOR]',
