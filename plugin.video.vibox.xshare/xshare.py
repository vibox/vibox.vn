...
def freeServers(name,url,img,fanart,mode,page,query):
	...
	if 'xemphimbox.com' in url:
		...
		if query=='eps':
			...
			link=''
			if '/openload.co/' in b:
				openloadAPI='https://api.openload.co/1/streaming/get?file='
				href=xsearch('(<iframe id="[^"]*player".+?/iframe>)',b)
				href=xsearch('src="(.+?)"',href).replace('&amp;','&')
				try:j=json.loads(xread(openloadAPI+xsearch('/([\w|-]{10,20})/',href)))
				except:j={}
				if j.get("status",0)==200:link=j.get('result',{}).get('url','')
			...
			if link:xbmcsetResolvedUrl(link)
			else:mess('File invalid or deleted!','phimlt.com')
...
def addir(name,link,img='',fanart='',mode=0,page=0,query='',isFolder=False,root=False):
	...
	li='plugin://plugin.video.vibox.xshare?'
...
def fshare_resolve(url,xml,name=''):
	...
	link = ""
	if fs.session_id and (not fs.acc or myaddon.getSetting("getLinkFree") == "true"):
		link = fs.getLink(url)
	...
	if link == "Failed" and "không tồn tại" in xread(url):
		...
		link = fs.getLinkFree(url, myaddon.getSetting("getLinkFree"))
...
def fsharePage(name,url,img,fanart,query=''):
	from resources.lib.servers import fshare;fs=fshare()
...
def fptplay(name,url,img,fanart,mode,page,query,text=''):
	...
	def fptVOD(url):
		...
		try:
			j = json.loads(xread('https://fptplay.net/show/timeshift',data=data))
		except:
			j = {}
		
		link = j.get("stream"," ")
		if link:xbmcsetResolvedUrl(link)
		else:mess('Get data link fail!') 
	...
	if filetime('fptplay.cookie') > 30:
		hd      = {'User-Agent':'Mozilla/5.0', 'Referer':'https://fptplay.net'}
		...
		if 'xshare' in b and 'laravel_id' in b:
			mess('Login to fptplay.net OK')
			xrw('fptplay.cookie', b.split('xshare')[1])
			xrw('fptplay.html', b)
	...
	if query=="fptplay.net":
		...
		if url == 'getSchedule' or url == 'i':
			if url == 'i':
				query = make_mySearch('schedule','https://fptplay.net/livetv','','','','Input')
			...
			if ':' in url:
				...
				all = False
			href = 'https://fptplay.net/show/schedule?channel=%s&date=%s&channel_now='%(url,t)
			#log(href)
...
def hayhaytv(name,url,img,fanart,mode,page,query):
	...
	if query=='hayhaytv.vn':
		...
		link=hh.getLink(url)
		if link:xbmcsetResolvedUrl(link)
		else:mess('Xin lỗi file không tồn tại hoặc đã bị xóa!','hayhaytv.vn') 
...
def hdonline(name,url,img,fanart,mode,page,query):
	...
	if query == 'hdonline.vn':
		...
		body  = get_home_page('hdonline.html',href,False,hd)
...
def vaphim(name,url,img,fanart,mode,page,query):
	...
	if query == 'vaphim.com':
		...
		query = make_mySearch('',url,'','','','Input')
		if query:
			return vaphim(name,url,img,fanart,mode,page,query)
		else:return 'no'
...
def bilutv(name,url,img,mode,page,query):
	...
	if query=='bilutv.com':
		...
		elif 'ì lý do bản quyền' in b:mess(u'Vì lý do bản quyền! BiluTV không cập nhật phim này nữa');return
...
def anime47(name,url,img,mode,page,query):
	...
	if query=='anime47.com':
		...
		if href:
			...
			hd   = {'User-Agent':'Mozilla/5.0','Referer':'http://anime47.com'}
			tk   = xsearch('\|(\w{32})\|',xread(href,hd))
		...
		if link:
			...
			mess(u'Xin lỗi! Link bạn muốn xem có thể đã bị hỏng')

def phimmedia(name,url,img,mode,page,query):
	ico=os.path.join(iconpath,'phim.png');urlhome='http://www.phim.media/';c='orange'
	if not os.path.isfile(ico):
	...
	def detail(s):
		...
		if re.search('\d/\d',name3):addir_info(namecolor(title,c),href,img,'',mode,1,'series',True)
	...
	if query=='phim.media':
		...
		addir_info(namecolor('Phim mới',c),'http://www.phim.media/phim-moi/',ico,'',mode,1,'page',True)
		...
		name=namecolor(name)
		body=make_request(url+'tap-1.html');art=''
		if not body:
			body=make_request(url)
			art=xsearch('<img alt=[^<]+?src="(.+?)"',body)
		...
		if len(re.findall('href="(.+?)" title="(.+?)"','\n'.join(i for i in items)))<30:
			for s in items:
				if len(items)>1:
					add_sep_Item(xsearch('<h4>(.+?)</h4>',s))
				for href,title in re.findall('href="(.+?)" title="(.+?)"',s):
					addir_info(title+' '+name,href,img,art,mode,1,'play');result=True
		
		elif len(items)>0:
			for s in items:
				label=xsearch('<h4>(.+?)</h4>',s)
				s='name="%s"'%name+s
				addir_info(namecolor(label,c),s,img,art,mode,1,'server',True);result=True
			
			s=items[0]
			add_sep_Item(xsearch('<h4>(.+?)</h4>',s))
			for href,title in re.findall('href="(.+?)" title="(.+?)"',s):
				addir_info(title+' '+name,href,img,art,mode,1,'play')
		...
		s=url
		add_sep_Item(xsearch('<h4>(.+?)</h4>',s))
		label=xsearch('name="(.+?)"',s)
		for href,title in re.findall('href="(.+?)" title="(.+?)"',s):
			addir_info(title+' '+label,href,img,fanart,mode,1,'play')
	
	elif query=='play':
		from resources.lib.servers import phimMedia;pmd=phimMedia()
...
def phimbathu(name,url,img,fanart,mode,page,query):
	...
	if query=='phimbathu.com':
		...
		for i in re.findall('(<li.+?/li>)',xread(href),re.DOTALL):
...
def tvhay(name,url,img,mode,page,query):
	...
	def get_Link(url,hd):
		...
		if len(tvhaytk) < 50 :
			...
			tvhaytk = xsearch('<title>Fshare - (.+?)</title>', tvhaytk).replace('amp;','')
		...
		data                     = 'link=%s&%s' % (linkData, tvhaytk)
		hd['Referer']            = url
...
def addir_info(name,url,img,fanart='',mode=0,page=1,query='',isFolder=False,
	...
	def get_mode(url,name,mode,query,isfolder=True):
		if 'fshare.vn/' in url and '/files/' not in url:
		...
		elif myaddon.getAddonInfo('id')!='plugin.video.vibox.xshare' : mode=0
			...
			name     = namecolor('Tenlua ', 'blue') + namecolor(name)
			mode     = 3
			isfolder = False
	...
	if menu:
		cmd   = 'RunPlugin(plugin://plugin.video.vibox.xshare?%s)'
		items = list()
...
def myFavourites(name,url,img,fanart,mode,page,query):
	def getFolders(folder,id):
		def getFolderItem(folder,id,result={}):
			for item in folder:
				if result:break
				elif item.get('id','')==id:result=item
				else:result=getFolderItem(item.get('folders',[]),id,result)
			return result
	...
	if userDefault and fs.user!=userDefault:fs=fshare();fs.user=userDefault
	...
	if query=='Home':
		if not fs.user or not fs.passwd:
			...
			id=accInfo.get("id")
			if not id:mess('Loads %s folder Failed!'%fs.user);return
			folders=root
		else:
			id=url
			folders=getFolders(root,id)
		
		b=xread('https://www.fshare.vn/folder/'+id,fs.hd)
		s='dl="/([^"]+?)"[^>]+?title="([^"]+?)">[^<]+?</a>'
		
		menu={'myFavourites':{'action':'MakeDeleteFolder'}}
		newsubfolder=False
		for id,title in [i for i in re.findall(s,b) if 'xshareFavourite ' in i[1]]:
		...
		if newsubfolder:xrw('fshare.json',json.dumps(fshareAccs))

		items=[i for i in re.findall(s,b) if re.search('\d{10}\.\d+',i[1])]
		...
		for timeID in sorted(timeSets,reverse=True):
			...
			try:title,href,img,fanart,m,p,q,folder=urllib2.base64.urlsafe_b64decode(s).split(',')
			except:continue
			folder=True if folder else False
	...
	if query=='fcine.net':
		...
		try:
			...
			if 'http://fcine.net' in url:
				...
				fn = xsearch('=(.+)',b.get('content-disposition',''))
			else:
				b  = xget(url)
...
def vungtv(name,url,img,fanart,mode,page,query):
	...
	if query=='vungtv.com':
		...
		href = xsearch('<iframe class="embed-responsive-item" src="(.+?)"',b)
		link = ''
		if href:
...
def banhtv(name,url,img,fanart,mode,page,query,text):
	...
	if query=='banhtv.com':
		...
		keyAES='banhtv.com4590481877'+id;j=[]
		dataLink=re.findall('data-link="(.+?)">(.+?)</a>',b)#;log(str(dataLink))
		if dataLink:
...
def kenhphimbo(name,url,img,fanart,mode,page,query):
	...
	if query=='kenhphimbo.net':
		...
		data = xsearch("_objPl.lk='(.+?)'", xread(url))
...
def animetvn(name,url,img,fanart,mode,page,query):
	urlHome='http://animetvn.com/';c='FFD0C101'
	...
	def pageNext(b):
		s=xsearch('(<div class="paging".+?/div>)',b)
		next=xsearch("<a title='Tiếp' href='(.+?)'>",s)
		if next:
			next='http://animetvn.com/'+next
			last=xsearch("<a title='Trang cuối' href='[^=]+?=(\d+)'>",s)
			title=namecolor('Trang tiếp theo ... %d/%s'%(page+1,last),'lime')
			addir_info(title,next,ico,'',mode,page+1,'page',True)
	
	def makeItems(s,label):
		add_sep_Item(label)
		items=re.findall('href="(.+?)" onclick=".*?" title=".+?" data-title="(.+?)"',s)
		for href,title in items:
			addir_info(s2c(title),href,img,'',mode,1,'play')
	
	def makeList(s):
		for m in re.findall('(<div class="items".+?/li>)',b,re.S):
			title=s2c(xsearch('title="(.+?)"',m))
			href=xsearch('href="(.+?)"',m)
			if not title or not href:continue
			img=xsearch('src="(.+?)"',m)
			label=re.sub('<.+?>','',xsearch('(<span class="time".+?/span>)',m))
			title+=' [COLOR FF13BBAF]%s[/COLOR]'%label
			label=re.sub('<.+?>','',xsearch('(<span class="year".+?/span>)',m))
			label=label+' '+re.sub('<.+?>','',xsearch('(<span class="viewed".+?/span>)',m))
			title+=' [COLOR green]%s[/COLOR]'%label
			addir_info(namecolor(title,c),href,img,'',mode,1,'eps',True)
	
	hd={'User-Agent':'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36','Cookie':xrw('animetvn.cookie')}
	if query=='animetvn.com':
		b=getHome('animetvn.html',urlHome)

		title=namecolor("Search trên animetvn.com","lime")
		addir_info(title,'animetvn.com',ico,'',mode,1,'search',True)
		
		addir_info(namecolor('Phim mới cập nhật',c),'phim-moi',ico,'',mode,1,'ajax',True)
		addir_info(namecolor('Phim lẻ',c),'phim-le',ico,'',mode,1,'ajax',True)
		addir_info(namecolor('Phim bộ',c),'phim-bo',ico,'',mode,1,'ajax',True)
		for label in re.findall('class="title">([^<]+?)</div>',b):
			addir_info(namecolor(label,c),'',ico,'',mode,1,'menu',True)
		
		try:tabs=dict(re.findall('href="#(tab-.)">(.+?)</a>',b))
		except:tabs={}
		for tab,rank in re.findall('href="#(tab-.)">(.+?)</a>',b):
			add_sep_Item('Bảng xếp hạng '+rank)
			s=xsearch('id="%s">(.+?/ul>)'%tab,b,1,re.S)
			for s in re.findall('(<li.+?/li>)',s,re.S):
				title=s2c(xsearch('>([^<]+?)</h3>',s))
				href=xsearch('href="(.+?)"',s)
				img=xsearch('src="(.+?)"',s)
				if not title or not href:continue
				label=xsearch('<b>(\d+)</b>',s)
				if label:title='[COLOR cyan]%s[/COLOR] '%label+title
				label=' '.join(re.findall('>([^<]+?)</font>',s))
				if label:title+=' [COLOR green]%s[/COLOR]'%label
				addir_info(namecolor(title,c),href,img,'',mode,1,'eps',True)
		...
		b=getHome('animetvn.html',urlHome)
		s=xsearch('(class="title">%s</div>.+?/ul>)'%namecolor(name),b,1,re.S)
		for href,title in re.findall('href="(.+?)">(.+?)</a>',s):
			addir_info(namecolor(title,c),href,img,'',mode,1,'page',True)
	
	elif query=='ajax':
		href='http://animetvn.com/ajax.php'
		data='home-film-tab=true&film=%s&page=%d'%(url,page)
		b=xsearch('(\{.+?\})',xread(href,hd,data))
		j=json.loads(b)
		b=j.get('html').encode('utf-8')
		makeList(b)
		last=xsearch("title='Trang cuối' href='#(\d+)'",j.get('pagination').encode('utf-8'))
		try:last=int(last)
		except:last=0
		if last > page:
			title=namecolor('Trang tiếp theo ... %d/%d'%(page+1,last),'lime')
			addir_info(title,url,ico,'',mode,page+1,query,True)
			
	elif query=='page':
		b=xreadc(url,hd)
		try:hd['Cookie']=b.split('xshare')[1];xrw('animetvn.cookie',hd['Cookie'])
		except:pass
		makeList(b)
		pageNext(b)
			
	#elif query=='eps':freeServers(name,'',img,fanart,mode,page,query)
	elif query=='eps':
		img=img
		b=xreadc(url,hd)
		try:hd['Cookie']=b.split('xshare')[1];xrw('animetvn.cookie',hd['Cookie'])
		except:pass
		img=xsearch('rel="image_src" href="(.+?)"',b)
		if xsearch('href="(.+?)"> *XEM ANIME',b):
			b=xread(xsearch('href="(.+?)"> *XEM ANIME',b),hd)
			#log(xsearch('href="(.+?)"> *XEM ANIME',b))
		
		name=xsearch('\](.+?)\[',name).strip()
		eplist=xsearch('(<div class="eplist" id=.+?/div>)',b,1,re.S)
		s = re.findall("(<span class='svep'.+?<br)",eplist,re.S)
		lenS=len(s)
		items=re.findall('href="(.+?)" onclick=".*?" title=".+?" data-title="(.+?)"',' '.join(s))

		pattern="<span class='svname_sub svname'>(.+?)</span>"
		if lenS>1 and len(items) > 30:
			xrw('eps.txt',eplist)
			for m in range(1,lenS):
				label=xsearch(pattern,s[m]).strip()
				eps=re.findall('>([^>]+?)</a>',s[m])
				eps=' [COLOR orange](%s - %s)[/COLOR]'%(eps[0],eps[len(eps)-1]) if eps else ''
				title='List phim trên server '+label+eps
				addir_info(namecolor(title,c),str(m),img,'',mode,1,'list',True)
			
			label=xsearch(pattern,s[0]).strip()
			makeItems(s[0],'List phim trên server '+label)

		elif lenS > 0:
			for s in s:
				label=xsearch(pattern,s).strip()
				makeItems(s,'List phim trên server '+label)
		
		s=xsearch('(<ul class="related_film player".+?/ul)',b)
		if s:
			s=''.join(i for i in re.findall('(<li.+?/li>)',s) if '"current"' not in i)
			add_sep_Item('Phim liên quan')
			for href,title in re.findall('href="(.+?)">(.+?)</a>',s):
				addir_info(namecolor(title,c),href,img,'',mode,1,'eps',True)

	#elif query=='list':freeServers('','','','','',1,'')
	elif query=='list':
		b=xrw('eps.txt')
		s = re.findall("(<span class='svep'.+?<br)",b,re.S)
		makeItems(s[int(url)],namecolor(name))
	
	elif query=='play':
		from resources.lib.utils import googleLinks
		link=googleLinks(xread(url,hd))
		if link:xbmcsetResolvedUrl(link)
		else:mess('File invalid or deleted!','animetvn.com')
	
def ffilm(name,url,img,fanart,mode,page,query):
	urlHome='http://fsharefilm.com/';c='FFF08080'
	...
	def itemsDir(s):
		...
		if label:title=title+' [COLOR green]%s[/COLOR]'%label
		if re.search('\d+/\d+|Full|tập|Tập',title):title=namecolor(s2c(title),c);dir=True;q='eps'
		else:title=s2c(title);dir=False;q='play'
		href=xhref(s)
		if not title or not href:return False
		img=ximg(s)
		addir_info(title,href,img,'',mode,1,'eps',True)
		return True
	...
	def getEPS(url):
		...
		if fshare:
			for href in fshare:addir_info(fn,href,img)
		...
		if s:
			...
			for i in re.findall('(<div class="movie.+?\s+</div>\s+</div>)',s,re.S):itemsDir(i)
	
	def fsharefilmPage(url):
		b=xread(url)
		s=xsearch('(<div class="list-movie.+?class="fb-page")',b,1,re.S)
		for i in re.findall('(<div class="movie.+?\s+</div>)',s,re.S):itemsDir(i)
		href=xsearch('<a href="([^"]+?)" *>&raquo;</a>',s)
		if href:
			try:pages=max(int(i) for i in re.findall('<a[^>]+?>(\d+?)</a></li>',s))
			except:pages=0
			title=namecolor('Trang tiếp theo... %d/%s'%(page+1,pages),'lime')
			addir_info(title,href,ico,'',mode,page+1,query,True)
	
	if query=='fsharefilm.com':
		b=getHome('fsharefilm.html',urlHome)

		title=namecolor("Search trên fsharefilm.com","lime")
		addir_info(title,'fsharefilm.com',ico,'',mode,1,'search',True)
		
		items=re.findall('<a href="(http://fsharefilm.com/chuyen-muc/.+?)">(.+?)</a>',b)
		hrefs=[]
		for href,title in items:
		...
		add_sep_item('Xem Nhiều Trong Tháng -----------------------------------')
		s=xsearch('(<h3 class="title">Xem Nhiều Trong Tháng.+?phimxemnhieu">)',b,1,re.S)
		items=re.findall('(<div class="movie.+?\s+</div>\s+</div>)',s,re.S)
		hrefs=[]
		if items:
			for s in items:
				r=itemsDir(s)
				if r:hrefs.append(xhref(s))
		
		add_sep_item('fsharefilm.com giới thiệu -----------------------------------')
		for s in re.findall('(<div class="slide.+?\s+</div>\s+</div>)',b,re.S):
			href=xhref(s)
			if href not in hrefs:
				r=itemsDir(s)
				if r:hrefs.append(xhref(s))
			
		add_sep_item('Phim Mới ----------------------------------------------------')
		for s in re.findall('(<div class="movie.+?\s+</div>)',b,re.S):
			href=xhref(s)
			if href not in hrefs:
				r=itemsDir(s)
				if r:hrefs.append(xhref(s))
		...
		b=xread(url)
		if xsearch('<a href="(.+?)"[^>]+>Xem Phim</a>',b):
			b=xread(xsearch('<a href="(.+?)"[^>]+>Xem Phim</a>',b))
		
		s=xsearch('(<section class="bx clearfix".+?/section>)',b,1,re.S)
		def x(i,s):
			if i=='url':return urllib.quote_plus(xsearch(" %s *= *'(.+?)'"%i,s))
			else:return xsearch(" %s *= *'(.+?)'"%i,s)
		url='%s?action=get_player&url=%s&sub=%s&subeng=%s&id_post=%s&id_ep=%s'
		url=url%(x('ajaxurl',s),x('url',s),x('sub',s),x('sub_eng',s),x('id_post',s),x('id_ep',s))
		b=xread(url)
		try:j=json.loads(xsearch('"linkdownload":(\{.+?\})',b))
		except:j={}
		items=[(i[1],i[0]) for i in j.items()]
		from resources.lib.utils import googleLinks
		link=googleLinks(items)
		if link:mess('Xshare playing downloads link')
		elif b:
