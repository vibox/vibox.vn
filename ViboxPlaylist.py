...
def GetSheetIDFromSettings():
	...
	'''
	sid = "11mNzl7tU6vvxAazkdOawC28JEt3x0YSrBLTYYoMMxeM"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			...
			if "spreadsheets/d/" in item["path"]:
				...
				item["path"] = "plugin://plugin.video.vibox.xshare/?mode=90&page=0&url=" + urllib.quote_plus(item["path"])
			elif any(service in item["path"] for service in ["4share.vn/d/"]):
				item["path"] = "plugin://plugin.video.vibox.xshare/?mode=38&page=0&url=" + urllib.quote_plus(item["path"])
			# elif any(service in item["path"] for service in ["4share.vn/f/"]):
				...
				item["path"] = "plugin://plugin.video.vibox.xshare/?mode=3&page=0&url=" + urllib.quote_plus(item["path"])
				item["is_playable"] = True
